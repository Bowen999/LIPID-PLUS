...
def predict_classes(model_package, input_path, output_path, ms1_tolerance_ppm=20, ms2_tolerance_ppm=20, use_rules=True):
    ...
    if single_overlap_mask.any():
        ...
        for idx in single_indices:
            ...
            # Determine confidence based on how the class was identified
            classes_mz = result.loc[idx, 'classes_mz']
            classes_ms2 = result.loc[idx, 'classes_ms2']
            ...
            if mz_empty and not ms2_empty:
                # Only classes_ms2 contributed
                result.loc[idx, 'class_confidence'] = 0.6
            elif ms2_empty and not mz_empty:
                # Only classes_mz contributed
                result.loc[idx, 'class_confidence'] = 0.5
            else:
                # Both contributed and they have one overlap
                result.loc[idx, 'class_confidence'] = 0.8
