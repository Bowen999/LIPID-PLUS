...
#     """
#     Process a single item from Msbuddy results for parallel processing.
    
#     :param item: Dictionary containing annotation results for one spectrum.
#     :return: Processed dictionary with extracted formula ranks.
#     """
#     processed_item = {
#         'index': str(item.get('identifier')),
#         'mz': item.get('mz')
#     }
#     # Extract all formula ranks
#     for i in range(1, 6):
...
#     """
#     Main function to run the lipid data processing pipeline.
...
#     mgf_path = os.path.join(result_path, "input.mgf")
#     buddy_result_path = os.path.join(result_path, "buddy_result.csv")
#     final_result_path = os.path.join(result_path, "formula_result.csv")

#     # --- 2. Convert CSV to MGF ---
#     print(f"Converting CSV '{csv_path}' to MGF format at '{mgf_path}'...")
#     try:
#         csv2mgf.convert_csv_to_mgf(csv_path, mgf_path)
#         print("MGF conversion successful.")
#     except Exception as e:
#         print(f"Error during CSV to MGF conversion: {e}")
#         return

#     # --- 3. Run Msbuddy Formula Annotation ---
#     print("Starting Msbuddy formula annotation...")
#     print(f"Using MS1 tolerance: {ms1_tol} ppm, MS2 tolerance: {ms2_tol} ppm.")

#     # Instantiate a MsbuddyConfig object
#     msb_config = MsbuddyConfig(
#         ms_instr=None,  # Use custom tolerances
#         ppm=True,
#         ms1_tol=ms1_tol,
#         ms2_tol=ms2_tol,
#         halogen=False
...
#     # Instantiate an Msbuddy object
#     msb_engine = Msbuddy(msb_config)

#     # Load data and annotate formulas
#     msb_engine.load_mgf(mgf_path)
#     msb_engine.annotate_formula()

#     # Retrieve the annotation result summary
#     buddy_summary = msb_engine.get_summary()
#     print("Msbuddy analysis complete.")

#     # --- 4. Process and Save Msbuddy Results ---
#     print("Processing Msbuddy results with parallel computing...")
    
#     # Use multiprocessing Pool for parallel processing
#     if len(buddy_summary) > 0:
#         # Only use parallelization if there are enough items
#         if len(buddy_summary) > n_cpus:
#             print(f"Processing {len(buddy_summary)} items using {n_cpus} CPU cores...")
#             with Pool(processes=n_cpus) as pool:
#                 processed_data = pool.map(process_single_item, buddy_summary)
#         else:
#             print(f"Processing {len(buddy_summary)} items sequentially (fewer items than CPU cores)...")
#             processed_data = [process_single_item(item) for item in buddy_summary]
#     else:
#         print("Warning: No items to process from Msbuddy results.")
#         processed_data = []

#     buddy_result_df = pd.DataFrame(processed_data)
...
#             print("Renaming 'formula_rank_1' column to 'formula'.")
